...
public class ConsultaBD {
    ...
    public ResultSet ConsultarItem(Context contexto, String item,String columna,bandera bandera)
    {


        resultado=null;
        try {
            resultado = ObtenerStatement(contexto).executeQuery("select I.CODCOL, I.CODTAL, I.DESCRIPCION,I.CODITM, I.CODMADRE, i.NOMBRECOL\n" +
                    "from pvitems as i where " + columna + "='" + item + "'" ); // columna para buscar por coditm o por codigo alternativo
            return resultado;

        } catch (SQLException e) {
            if(stmt!=null) {
                bandera.valor=2;
                bandera.setValor(2);// 1 significa que no hay conexion
            }
    ...
    public ResultSet ConsultarPrecioItem(Context contexto, String buscar)
    {

        resultado=null;
        try {
            resultado = ObtenerStatement(contexto).executeQuery("select lp.PRECIO\n" +
                    "from pvListasprecios as lp where lp.coditm = '" + buscar + "'\n" +
                    "and lp.CODLIS = (select STRVALOR from pvconfig where PROPIEDAD = 'listapreciopublico')");
            return resultado;
    ...
    public ResultSet ConsultarStock(Context contexto, String buscar)
    {
        resultado=null;
        try {
            resultado = ObtenerStatement(contexto).executeQuery("select (ia.STKACTUAL - [dbo].[FN_STOCK_EN_BULTOS]('" + buscar +"')\n" +
                    "- [dbo].[FN_STOCK_SENIADO]('" + buscar +"') - [dbo].[FN_STOCK_PE]('" + buscar +"',1)) as STKACTUAL\n" +
                    "from PVItemsAcum as ia where coditm='"+ buscar +"' \n" +
                    "and ia.CODSUC = (select intvalor from pvconfig where PROPIEDAD = 'codsuc')");
            return resultado;
...
//            configuracion.Configuracion(contexto);
            resultado=null;
     // 27/07/2020 select nuevo donde traiga todos los productos que tengan ese codigo madre
            //  resulset
            resultado = ObtenerStatement(contexto).executeQuery(
    ...
    public ResultSet ObtenerValorCodigo(Context contexto, String buscar,String codlista,bandera bandera)

    {
        try {
            ...
            {
                resultado = ConsultarItem(contexto, reemplazado,"coditm",bandera);
                ...
                if(!resultado.isBeforeFirst())
                {
                    ...
                    resultado = ConsultarItem(contexto, reemplazado,"CODITMALTERNATIVO",bandera);
                    if (resultado.isBeforeFirst())
                    {
            ...
            if( reemplazado.compareTo(original) == 0 )
            {
                ...
                resultado = ConsultarItem(contexto, original,"CODITMALTERNATIVO",bandera);
                ...
                if(!resultado.isBeforeFirst())
                {
                    resultado = ConsultarItem(contexto, original,"CODITM",bandera);;
